/**
 * MESSAGING SYSTEM - RENDER OPTIMIZED (Auto-Reconnect)
...
const io = new Server(server, {
  pingTimeout: 60000, // Wait 60s before assuming connection is dead
...
<head>
    ...
    <style>
        ...
        .sidebar { width: 260px; background: var(--sidebar-bg); border-right: 1px solid var(--secondary); display: flex; flex-direction: column; padding: 15px; }
        .main-content { flex: 1; display: flex; flex-direction: column; background: var(--bg-color); position: relative; }

        button { background: var(--primary); color: white; border: none; padding: 10px; cursor: pointer; border-radius: 5px; margin: 5px 0; font-weight: bold; }
        ...
        .login-box { background: var(--secondary); padding: 40px; border-radius: 10px; width: 300px; text-align: center; border: 1px solid var(--primary); }
        ...
        .section-title { color: var(--accent); margin-top: 20px; font-size: 0.9em; text-transform: uppercase; letter-spacing: 1px; }
        ...
        /* Reconnect Banner */
        #reconnect-banner { position: absolute; top: 0; left: 0; width: 100%; background: #cc3300; color: white; text-align: center; padding: 5px; font-size: 0.8em; display: none; z-index: 999; }
    </style>
</head>
<body>
        ...
        <div class="main-content">
            ...
            <div id="chat-screen">
                <div class="chat-header">
                    ...
                    <button onclick="leaveRoom()" style="background: #cc3300; font-size: 0.8em;">Exit</button>
                </div>
    ...
    <script>
        ...
        // SESSION STATE (To survive reconnects)
        let myData = {
            name: '',
            pfp: '',
            currentRoomId: null,
            roomPassword: null // Temporarily store pass if we need to rejoin
        };
        ...
        socket.on('connect', () => {
            ...
            // If we have a name, we were logged in before. Re-login automatically.
            if(myData.name) {
                console.log("Reconnecting... Restoring session.");
                socket.emit('login', { name: myData.name, pfp: myData.pfp });
            }
        });

        // Need to wait for login_success before trying to re-join room
        socket.on('login_success', (user) => {
            // Update local state if it was a fresh login
            if(!myData.name) {
            ...
            if(myData.currentRoomId) {
                socket.emit('join_room', { roomId: myData.currentRoomId, password: myData.roomPassword });
            }
        });
        ...
        function editProfile() {
            const newName = prompt("New Name:", myData.name);
            if(newName) {
                const newPfp = prompt("New PFP URL:");
                myData.name = newName; // Update local
                if(newPfp) myData.pfp = newPfp;
                socket.emit('update_profile', { name: newName, pfp: newPfp || myData.pfp });
            }
        }
        ...
        socket.on('update_room_list', (rooms) => {
            ...
            rooms.forEach(room => {
                ...
                div.innerHTML = \`<span>\${room.name} \${room.hasPassword ? '🔒' : ''}</span> <button style="font-size:0.6em; padding:4px 8px;" onclick="joinRoom('\${room.id}', \${room.hasPassword})">Join</button>\`;
        ...
        socket.on('update_user_list', (users) => {
            ...
            users.forEach(u => {
                ...
                div.innerHTML = \`<div style="display:flex; align-items:center; gap:8px;"><img src="\${u.pfp}" style="width:25px; height:25px; border-radius:50%; object-fit:cover;"><span>\${u.name}</span></div><button style="font-size:0.6em; padding:4px 8px; background: var(--accent);" onclick="startPM('\${u.id}')">Msg</button>\`;
                container.appendChild(div);
            });
        });
        ...
        function joinRoom(id, hasPass) {
            let password = '';
            if(hasPass) password = prompt("Enter Room Password:");
            myData.roomPassword = password; // Save for reconnects
            socket.emit('join_room', { roomId: id, password });
        }

        function startPM(targetId) { socket.emit('start_pm', targetId); }
        socket.on('request_pm_join', (roomId) => { socket.emit('join_room', { roomId: roomId }); });
        socket.on('join_pm_success', (roomId) => { socket.emit('join_room', { roomId: roomId }); });

        // --- CHAT UI ---
        socket.on('joined_room', (roomData) => {
            myData.currentRoomId = roomData.id; // Save state
        ...
        socket.on('room_users', (users) => {
            ...
            users.forEach(u => {
                const div = document.createElement('div');
                div.style.background = '#444'; div.style.padding = '3px 8px'; div.style.borderRadius = '10px'; div.style.display = 'flex'; div.style.alignItems = 'center'; div.style.gap = '5px';
                ...
                if(iAmCreator && u.id !== socket.id) {
                    const kickBtn = document.createElement('span'); kickBtn.innerText = ' ✖'; kickBtn.style.color = '#ff4444'; kickBtn.style.cursor = 'pointer'; kickBtn.onclick = () => { if(confirm(\`Kick \${u.name}?\`)) socket.emit('kick_user', u.id); };
        ...
        socket.on('chat_message', (msg) => {
            ...
            div.innerHTML = \`<div class="meta"><img src="\${msg.user.pfp}"><span>\${msg.user.name}</span><span style="margin-left:auto; font-size:0.8em;">\${msg.time}</span></div><div class="text">\${msg.text}</div>\`;
        ...
        socket.on('system_message', (text) => {
            const div = document.createElement('div'); div.style.textAlign = 'center'; div.style.color = 'var(--accent)'; div.style.fontSize = '0.75em'; div.style.margin = '10px 0'; div.innerText = text;
            document.getElementById('chat-messages').appendChild(div);
        });

        socket.on('error_msg', (msg) => { alert(msg); });
        socket.on('kicked', () => { leaveRoomUI(); });
